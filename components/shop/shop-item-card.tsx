  ...
  return (
    <>
      ...
      >
        ...
        <p className="text-sm text-neutral-300 mb-4 min-h-[40px] line-clamp-2">
          {item.description}
        </p>
        
        {item.effect_type && (
          <div className="text-xs text-neutral-400 mb-4 bg-neutral-900/50 rounded px-2 py-1.5 border border-neutral-700">
          ...
          <div className="flex flex-col gap-1">
            {item.cost_gold > 0 && (
              <div className={`flex items-center gap-1.5 font-bold ${
                userGold >= item.cost_gold ? 'text-amber-400' : 'text-red-500'
              }`}>
                <span>🪙</span>
            ...
            {item.cost_gems > 0 && (
              <div className={`flex items-center gap-1.5 font-bold ${
                userGems >= item.cost_gems ? 'text-cyan-400' : 'text-red-500'
              }`}>
                <span>💎</span>
        ...
        <DialogContent className="bg-neutral-900 border-neutral-700">
          <DialogHeader>
            ...
            <DialogDescription className="text-neutral-400">
              Are you sure you want to purchase <span className="text-white font-semibold">{item.name}</span>?
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            <div className="bg-neutral-800 rounded-lg p-4 space-y-2">
              ...
              {item.effect_type && (
                <div className="flex justify-between text-sm">
                  <span className="text-neutral-400">Effect:</span>
                  <span className="text-white">
                    {item.effect_type.replace(/_/g, ' ')}
                    {item.effect_value && ` ${item.effect_value}x`}
                  </span>
            ...
            <div className="bg-neutral-800 rounded-lg p-4 space-y-2">
              <div className="text-sm font-semibold text-white mb-2">Cost:</div>
              {item.cost_gold > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-neutral-400 flex items-center gap-1.5">
                    <span>🪙</span> Gold
                  </span>
                  <span className="text-amber-400 font-bold">-{item.cost_gold.toLocaleString()}</span>
                </div>
              )}
              {item.cost_gems > 0 && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-cyan-400 font-bold">-{item.cost_gems.toLocaleString()}</span>
                </div>
              )}
              ...
              <div className="border-t border-neutral-700 pt-2 mt-2">
                {item.cost_gold > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-neutral-400">New Gold Balance:</span>
                ...
                {item.cost_gems > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-neutral-400">New Gem Balance:</span>
                    <span className="text-white font-semibold">
